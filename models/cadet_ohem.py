...
class cadet(nn.Module):
    ...
    @staticmethod
    def _ohem_bce_loss(logits, targets, keep_ratio: float = 0.25):
        """
        logits  : (N,1,H,W) raw scores（未过 sigmoid）
        targets : (N,1,H,W) float / long, 0 or 1
        keep_ratio : 取前 P% 高损失像素
        """
        # 逐像素 BCE（不做 mean）
        loss_map = F.binary_cross_entropy(logits, targets, reduction='none')                                  # (N,1,H,W)

        # 拉平成 1-D，再选 top-k
        loss_flat = loss_map.view(-1)
        k         = max(int(loss_flat.numel() * keep_ratio), 1)
        hard_loss, _ = torch.topk(loss_flat, k=k, sorted=False)
    ...
    def compute_objective(
        ...
        Args
        ----
        use_ohem   : True → 使用 Online Hard Example Mining
        ohem_ratio : OHEM 保留的像素比例（0~1）
        """
        loss_dict  = {}
        gt_mask    = batch_dict['anno_mask']       # (N,1,H,W)
        gt_boundary= batch_dict['anno_boundary']   # (N,1,H,W)
        h, w       = gt_mask.shape[2:]

        total_loss = None
        for i in range(self.steps):
            pred_mask     = output_dict[f'step_{i}_output_mask']      # list
            pred_boundary = output_dict[f'step_{i}_output_boundary']  # list

            # -------- 1) Mask loss -------- #
            step_mask_loss = 0.0
            for inner_pred in (pred_mask if multi_layer else pred_mask[:1]):
                if inner_pred is None:              # 跳过 None 层
                    continue
                inner_pred = F.interpolate(
                    inner_pred, (h, w), mode='bilinear', align_corners=True
                )
                if use_ohem:
                    mask_loss = self._ohem_bce_loss(inner_pred, gt_mask.float(), ohem_ratio)
                else:
                    mask_loss = F.binary_cross_entropy_with_logits(
                        inner_pred, gt_mask.float(), reduction='mean'
                    )
                step_mask_loss += mask_loss
            step_mask_loss /= len(pred_mask) if multi_layer else 1

            # -------- 2) Topology / boundary loss -------- #
            step_topo_loss = 0.0
            for inner_pred in (pred_boundary if multi_layer else pred_boundary[:1]):
                ...
                if use_ohem:
                    topo_loss = self._ohem_bce_loss(inner_pred, gt_boundary.float(), ohem_ratio)
                else:
                    topo_loss = F.binary_cross_entropy_with_logits(
                        inner_pred, gt_boundary.float(), reduction='mean'
                    )
                step_topo_loss += topo_loss
            # 若所有层都 None，则 step_topo_loss 仍为 0
            if multi_layer:
                step_topo_loss /= max(len(pred_boundary), 1)

            # -------- 3) 累加 / 记录 -------- #
            if total_loss is None:
                total_loss = torch.zeros_like(step_mask_loss)

            total_loss = total_loss + step_mask_loss + step_topo_loss
            loss_dict[f'step_{i}_total_loss'] = step_mask_loss + step_topo_loss
